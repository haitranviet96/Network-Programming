...
void print_games(game_t tab_p[], int n) {
    ...
    for (i = 0; i < n; ++i) {
        printf("%d\t| %s\t| %s\t| %d\t| %s\n", i, tab_p[i].name, inet_ntoa(tab_p[i].addr.sin_addr),
               ntohs(tab_p[i].addr.sin_port), (tab_p[i].status == AVAILABLE_ST) ? "Yes" : "No");
    }
}
...
void accept_player(int sfd_l, opponent_t *opponent_p) {
    ...
    check(opponent_p->sfd = accept(sfd_l, (struct sockaddr *) &(opponent_p->addr), &(opponent_p->addr_len)),
          "Error accepting");
}
...
int sendFire(int sfd_o, char *message) {
    char buff[MAX_REQ] = "";
    char cmd[10] = "";

    sprintf(buff, "FIRE %d %d", target.column, target.row);
...
int returnFire(int sfd_o, char* cmd, char *message) {
    char buff[MAX_REQ] = "";

    sprintf(buff, "%s %s", cmd, message);
